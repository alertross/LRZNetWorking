...
#pragma mark - LRZURLRequest
@implementation LRZURLRequest
- (void)dealloc{
    LRZLog(@"%s",__func__);
}
...
- (void)addFormDataWithName:(NSString *)name fileData:(NSData *)fileData {
    LRZUploadData *formData = [LRZUploadData formDataWithName:name fileData:fileData];
    [self.uploadDatas addObject:formData];
}

- (void)addFormDataWithName:(NSString *)name fileName:(NSString *)fileName mimeType:(NSString *)mimeType fileData:(NSData *)fileData {
    LRZUploadData *formData = [LRZUploadData formDataWithName:name fileName:fileName mimeType:mimeType fileData:fileData];
    [self.uploadDatas addObject:formData];
}

- (void)addFormDataWithName:(NSString *)name fileURL:(NSURL *)fileURL {
    LRZUploadData *formData = [LRZUploadData formDataWithName:name fileURL:fileURL];
    [self.uploadDatas addObject:formData];
}

- (void)addFormDataWithName:(NSString *)name fileName:(NSString *)fileName mimeType:(NSString *)mimeType fileURL:(NSURL *)fileURL {
    LRZUploadData *formData = [LRZUploadData formDataWithName:name fileName:fileName mimeType:mimeType fileURL:fileURL];
    [self.uploadDatas addObject:formData];
}
...
- (NSMutableArray<LRZUploadData *> *)uploadDatas {
    if (!_uploadDatas) {
...
#pragma mark - LRZBatchRequest

@interface LRZBatchRequest()
/**
 *  离线下载栏目url容器
 */
...
@implementation LRZBatchRequest
...
- (void)cancelbatchRequestWithCompletion:(cancelCompletedBlock)completion{
    if (_urlArray.count > 0) {
        [_urlArray enumerateObjectsUsingBlock:^(LRZURLRequest *request, NSUInteger idx, __unused BOOL *stop) {
            [LRZRequestManager cancelRequest:request.URLString completion:completion];
        }];
    }
}
...
#pragma mark - LRZUploadData

@implementation LRZUploadData

+ (instancetype)formDataWithName:(NSString *)name fileData:(NSData *)fileData {
    LRZUploadData *formData = [[LRZUploadData alloc] init];
    formData.name = name;
